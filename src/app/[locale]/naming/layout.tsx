...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const t = await getTranslations({ locale: params.locale, namespace: "naming" });
  const base = BASE_URL;
  const path = params.locale === "en" ? "/naming" : `/${params.locale}/naming`;
  ...
  return {
    ...
    openGraph: {
      ...
      type: "website",
      images: [{ url: `${base}/api/og?title=${encodeURIComponent(t("meta.ogSub"))}&sub=${encodeURIComponent(t("meta.ogSub"))}&lang=${params.locale}`, width: 1200, height: 630 }],
    },
    twitter: {
    ...
    <>
      <BreadcrumbListSchema
        items={[
          ...
          { name: "Create a Chinese Name", url: `${BASE_URL}/naming` },
        ]}
      />
      {children}
    </>
