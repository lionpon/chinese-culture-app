...
> ⚠️ **立项定位：不针对大陆买家（永久排除）**。所有支付/推广/合规方案一律不考虑大陆消费者，不要提支付宝/微信/大陆收单等建议。
...
> **收款方式决策（2026-08-28，不再讨论）**：
> - ❌ **不加加密货币收款（含 USDT）**：大陆主体操作者的合规/法律风险（冻卡、行政刑事红线）远大于 RU 客群的边际收益；$1 微支付场景加密币 UX 不成立。USDT 尤其排除——国内电诈资金首选载体，OTC 换汇冻卡率最高，稳定币不豁免 2021 禁令
...
### 合规红线
- 内容一律标注 "for entertainment purposes only"
- AI 生成内容按平台要求打 AI 标签
- 不做 24h 循环直播（YouTube spam 政策），定时 2-4h/场
- 社媒引流统一带 UTM 参数，验证 referrer 归因修复（8/21）效果

### 决策记录
- 数字人直播**不先于录播**：无粉丝基数时直播零观众，先用录播测内容吸引力
- 国内平台数字人直播 = 方向性错误（客群不匹配），永久排除
- 支付/加密币/大陆收单已另行定案（见「支付流程」章节）
...
- 验证：E2E 扩至 28 项（email 采集→继承→付费墙兜底→挽回邮件跳过安全）全过；补充套件 18 项（付费墙 email 兜底、IPN/PDT 全路径、三表单 UI 采集）全过；tsc/lint/build 全绿
- ✅ **RESEND 已确认配置**（9/17 生产 cron 探测返回 `email: "sent"`）——挽回邮件会真实发出，下个弃单用户 24h 后即收到；首封发出后看 cron JSON 的 `recoverySent` 字段确认
...
3. ~~大陆访客付款~~ **不适用**：大陆买家不在目标客群（立项定位，永久排除）
4. **9 月底评估 Starter 是否续费**（转化是否破零；破零即续，否则降级 + 迁 Vercel 免费档备选）
5. **营销开工（下次会话）**：第 1 步 content-factory skill + 英语日期长尾页第一批；第 2 步 Twitter/TG 每日一卦自动化 + Pinterest 图钉流；~9/11 启动第 3 步数字人录播（详见「营销与增长」章节）
