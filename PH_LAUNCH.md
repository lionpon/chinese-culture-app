# 🚀 Product Hunt 发布计划

> **目标日期**：2026-07-23 周四 | **产品**：Chinese Culture Studio | **状态**：✅ 已预约

---

## 一、PH 发布文案

### Tagline（60字符内）

```
5 Ancient Chinese Wisdom Tools — Name, Dates, I Ching, Palm & Dream. Pay What You Want.
```
（需确认最终用的是什么）

---

### Description

```
Chinese Culture Studio brings 5 ancient Chinese wisdom practices into one beautiful, AI-powered experience — for just $1.

🔮 WHAT YOU GET:
• Create a Chinese Name — based on Bazi (八字) and Five Elements
• Auspicious Date Selection — Chinese almanac for weddings, business, travel & 10+ events
• I Ching Divination — the 3000-year-old Book of Changes oracle (time/random/manual)
• AI Palm Reading — upload hand photo, get instant classical analysis
• AI Dream Interpretation — decode dreams with Chinese dream dictionary

✨ WHY IT'S SPECIAL:
• Real classical Chinese texts (I Ching, 三命通会, 协纪辨方书)
• Instant results — no wait, no signup
• $1 flat — no subscription, pay only for full results
• 4 languages: English · Русский · 日本語 · 한국어
• Used in 42+ countries worldwide

Try free, unlock full reading for $1. No account needed.
```

---

### First Comment（Maker 第一条评论）

```
Hey Product Hunt! 👋

Solo maker here. Built Chinese Culture Studio because most "Chinese astrology" sites are either generic AI wrappers or cost $5-$35 per reading. I wanted something authentic, affordable, and clean.

Every reading uses real classical texts. Pay what you want, starting at $1 — no subscription traps. Free tier so anyone can try first.

📊 26+ countries, fully organic traffic
🔧 Next.js 14 · TypeScript · Prisma · Supabase · Tailwind · PayPal · OpenRouter AI

💡 Fun fact: The I Ching oracle correctly predicted Spain would win the World Cup! I built a live prediction page during the tournament.

🚀 Would love your feedback:
- Is pay-what-you-want the right model?
- Which service interests you most?
- What feature next? (Feng Shui? Face Reading? Bazi compatibility?)

Try it: https://www.culture-of-china.com
Thank you! 🙏
```

---

### 分类

- **主分类**：Web App 或 AI
- **Topic**：Tech

---

### 图片素材（使用已有截图）

| 顺序 | 文件 | 说明 |
|------|------|------|
| 1 | `public/screenshot-home.png` | 首页 |
| 2 | `public/screenshot-naming.png` | 起名 |
| 3 | `public/screenshot-divination.png` | 占卜 |
| 4 | `public/screenshot-guide.png` | 指南 |
| 5 | `public/logo.png` | Logo |

---

### 链接

| 项 | URL |
|----|-----|
| Website | https://www.culture-of-china.com |
| GitHub | https://github.com/lionpon/chinese-culture-app |

---

## 二、发布前检查清单

### 🔴 Must Do

- [x] 注册 PH 账号 → https://www.producthunt.com
- [x] 创建产品 → https://www.producthunt.com/launch
- [x] 选择发布日 → 2026-07-23 00:01 PST (北京时间 15:01)
- [x] Tags: Productivity
- [x] Funding: Bootstrapped
- [x] Gallery 截图上传（首页/取名/起卦/AI解梦）
- [x] Shoutouts 已填
- [x] First Comment 已准备
- [x] 投资者部分已填
- [ ] 提前 30min 唤醒 Render（手动访问网站）

### 🟡 Nice to Have

- [ ] 网站加 PH 投票 Badge
- [ ] 30-60s Demo 视频
- [ ] Twitter 提前预告

### 定价确认

- 预设档位：$1 · $3 · $5 · $10
- 自定义金额：$1 ~ $100 自由输入
- 最小值 $1，最大值 $100
- 免费试用：每人 1 次

---

## 三、发布日时间线（北京时间）

| 时间 | 操作 |
|------|------|
| 14:30 | 唤醒 Render + 检查支付/工具 |
| 15:01 | PH 自动上线 |
| 15:05 | 发布 First Comment |
| 15:05-16:00 | 转发 Twitter / Reddit / HN |
| 全天 | 回复每一条 PH 评论 |

---

## 四、发布后追踪

| 指标 | 来源 |
|------|------|
| PH 访问量 | PH 后台 |
| 网站 UV/PV | /api/stats |
| 工具使用量 | /api/track 埋点 |
| 支付转化 | Purchase 表 |
| 邮件订阅 | Subscriber 表 |
